(*Two day extension granted*)

(* A.5 
   We don't have to install the environment in the LetEnvFrame before executing the 
   exception handler because the handler is a function and function definitions 
   have their own enviroments so if a exception is eventually thrown it executes in the 
   context of the enviroment that the handler was made in.
*)
  ...
  if !stack_tracing > 1 then
    ...
    if stack_is_empty context then stack_push l frame context else
      match (frame, stack_top context) with
        | (CallEnvFrame _, CallEnvFrame _)
        | (LetEnvFrame  _, CallEnvFrame _) 
        | (LetEnvFrame  _, LetEnvFrame _) -> context
        | (CallEnvFrame _, LetEnvFrame y) -> 
          let (_, st) = (stack_pop context) in
            (stack_push l (CallEnvFrame y) st)
...
let eval_val_in_frame v env frame context =
  match frame with
    ...
    | BeginFrame (l, e :: es) ->
      if es = [] then (Expr e, env, context)
      else let context' = stack_push l (BeginFrame (l, es)) context in
        (Expr e, env, context')
    ...
    | ReturnFrame l ->
      let rec iter ev cont =
        if stack_is_empty cont then Error.return_outside_func l 
        else match (stack_pop cont) with
          | (CallEnvFrame x, cont') -> (Value v, x, cont')
          | (_, cont') -> iter ev cont'
      in iter env context
    ...
    | TryCatchEvalHandlerFrame (l, t) -> 
      begin
        match v with
          | PrimFuncVal _
          | UserFuncVal _ ->
            let cont = push_env_frame l (LetEnvFrame env) context in
            (Expr t, env, stack_push l (TryCatchFrame v) cont)
          | _ -> Error.type_err l 
                   ~expected: "function" 
    ...
    | ThrowFrame l -> 
      let rec iter ev cont =
        if stack_is_empty cont then Error.uncaught_throw l "empty stack" else
          match (stack_pop cont) with
            | (TryCatchFrame x, c) -> eval_call l x [v] env c
            | (_, c) -> iter ev c
      in iter env context
...
let reduce_expr e env context =
  match e with
    ...
    | Throw (l, e') -> (Expr e', env, stack_push l (ThrowFrame l) context)

    (* `break` is special: it doesn't need its own frame because it doesn't take
     * an argument, so it can be evaluated immediately.  Just unwind the stack to
     * the last `while` loop. *)
    | Break l -> 
      let rec iter ev cont = 
        if stack_is_empty cont then Error.break_outside_while l 
        else match (stack_pop cont) with
          | (CallEnvFrame _, _)  -> Error.break_outside_while l
          | (LetEnvFrame x, cont') -> iter x cont'
          | (WhileRunningBodyFrame _, cont') -> (Value UnitVal, ev, cont')
          | (_, cont') -> iter ev cont'
      in iter env context

    | Continue l -> 
      let rec iter ev cont = 
        if stack_is_empty cont then Error.continue_outside_while l 
        else match (stack_pop cont) with
          | (CallEnvFrame _, _)  -> Error.continue_outside_while l
          | (LetEnvFrame x, cont') -> iter x cont'
          | (WhileRunningBodyFrame (lo, ep1, ep), cont') -> 
              (Expr ep1, ev, stack_push l (WhileFrame (lo, ep1, ep)) cont')
          | (_, cont') -> iter ev cont'
      in iter env context
