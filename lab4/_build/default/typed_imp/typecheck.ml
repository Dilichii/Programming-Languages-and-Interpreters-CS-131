...
let rec string_of_type = function
  ...
  | UnitType    -> "unit"
  | BoolType    -> "bool"
  | ArrayType x -> "(array " ^ (string_of_type x) ^ ")"
...
let get_arraytype l f x = 
  match (f x) with
  | ArrayType s -> s
  | _ -> Error.type_err l ~expected:"ArrayType" ~found:(string_of_type (f x))

let is_arraytype l f x = 
  match (f x) with
  | ArrayType _ -> ()
  | _ -> Error.type_err l ~expected:"ArrayType" ~found:(string_of_type (f x))
...
let rec ty_eq t1 t2 = 
  match (t1, t2) with
    ...
    | (ArrayType a1, ArrayType a2) -> (ty_eq a1 a2)
    | _ -> false
...
let rec typecheck_expr env exp =
  ...
  match exp with
    ...
    | ArrayMake  (l, e1, e2) ->
      begin
        assert_ty_eq l (typecheck e1) IntType;
        let tau = typecheck e2 in
        ArrayType (tau)
      end 

    | ArrayAt    (l, arr, e1) -> 
      begin
        is_arraytype l typecheck arr; 
        let x = get_arraytype l typecheck arr in
        begin 
          assert_ty_eq l (typecheck e1) IntType;
          x
        end
