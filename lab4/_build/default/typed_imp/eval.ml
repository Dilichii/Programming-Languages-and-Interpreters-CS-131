...
let to_int l v = 
  match v with 
  | Value.Int i -> i
  | _ -> Error.bug_in_type_checker_err l "input not an Int Value"

let to_array l v =
  match v with 
  | Value.Array a -> a
  | _ -> Error.bug_in_type_checker_err l "input not an Array Value"
...
let rec eval_expr env exp =
  match exp with
    ...
    | ArrayMake  (l, e1, e2) -> 
      ...
      (env'', Value.Array (Array.make (to_int l v1) v2))           
      
    | ArrayAt    (l, arr, e1) -> 
      let (env', arr') = eval_expr env arr in
      let (env'', v1) = eval_expr env' e1 in
      let varr = to_array l arr' and vlength = to_int l v1 in 
      if (vlength < Array.length varr) && (vlength >= 0) then
        (env'', Array.get varr vlength)
      else Error.index_err l vlength

    | ArrayPut   (l, arr, e1, e2) ->
      let (env', arr') = eval_expr env arr in
      let (env'', v1) = eval_expr env' e1 in
      let (env''', v2) = eval_expr env'' e2 in
      let varr = to_array l arr' and vi = to_int l v1 in
      if (vi < Array.length varr) && (vi >= 0) then
        begin
          Array.set varr vi v2;
          (env''', Value.Unit)
        end
      else Error.index_err l vi
      
    | ArraySize  (l, arr) ->
      let (env', arr') = eval_expr env arr in
      (env', Value.Int (Array.length (to_array l arr')))
...
let rec default_value_for_type x = 
  match x with 
  | UnitType -> Value.Unit
  | BoolType -> Value.Bool false
  | IntType -> Value.Int 0
  | ArrayType a -> Value.Array (Array.make 0 (default_value_for_type a))
...
let rec eval_def ({type_env; val_env} as env) def =
  match def with
    ...
    | Define (l, ({name; formals; locals; body; _} as decl)) ->
      ...
      let fill = List.map (fun (i, t) -> (i, default_value_for_type t)) locals in
      let func = Value.UserFunction (List.map fst formals, fill, body) in
