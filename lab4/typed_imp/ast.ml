...
let rec parse_expr = function
  ...
  | List (l, [Id (_, "make-array"); e1; v1]) -> ArrayMake (l, parse_expr e1, parse_expr v1)

  | List (l, [Id (_, "array-at"); arr; inx]) ->
     ArrayAt (l, parse_expr arr, parse_expr inx)

  | List (l, [Id (_, "array-put"); arr; inx; v1]) ->
    ArrayPut (l, parse_expr arr, parse_expr inx, parse_expr v1)

  | List (l, [Id (_, "array-size"); arr]) ->
    ArraySize (l, parse_expr arr)

  | List (l, (Id (_, "make-array") :: _)) -> error l "invalid \"make-array\""

  | List (l, (Id (_, "array-at") :: _)) -> error l "invalid \"make-at\""

  | List (l, (Id (_, "array-put") :: _)) -> error l "invalid \"make-put\""

  | List (l, (Id (_, "array-size") :: _)) -> error l "invalid \"make-size\""
...
let rec parse_type = function
  ...
  | Id (_, "bool") -> BoolType
  | Id (_, "int")  -> IntType
  | List (_, [Id (_, "array"); x]) -> ArrayType (parse_type x)
...
let parse_def = function
  ...
  | List (l, [Id (_, "define"); ty; Id (_, name); List (_, formals); body]) ->
    ...
    let formal_names = List.map fst formals' in
      if unique_ids formal_names then
        ...
        error l "non-unique formal parameter names"

  | List (l, [Id (_, "define"); ty; Id (_, name); List (_, formals);
       List (_, Id (_, "locals") :: loclst); body]) ->
    (* Convert the formal parameters to strings. *)
    let parse_formal =
      let err l = error l "bad argument name" in
        function
          | List (l, [Id (_, name); Id (_, ":"); ty]) -> 
            let name' = validate_name l name in
              (name', parse_type ty)
    ...
    in
    let formals' = List.map parse_formal formals in
    let loclst' = List.map parse_formal loclst in
    let ret      = parse_type ty in
    let body     = parse_expr body in

    (* Check that the formal parameter names are unique. *)
    let formal_names = List.map fst formals' and
    locals_names = List.map fst loclst' in
      if (unique_ids formal_names) && (unique_ids locals_names) then
        Define (l, {ret; name; formals=formals'; locals = loclst'; body})
      else
        error l "non-unique argument names"
