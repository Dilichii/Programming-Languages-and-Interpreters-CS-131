...
let read = PrimFunction
(fun l args ->
    if args = [] then
      match (read_int_opt ()) with 
                | None -> raise (Runtime_err (l, "Input cannot be converted"))
                | Some n -> n
    else raise (Call_err (l, "read", 0, List.length args)))
...
let keywords = 
  ["val"; "define"; "use";
   ...
   "set"; "if"; "while"; "begin"; "do-while";
   "while*"; "for"; "and"; "or"; "locals"]
...
let rec parse_expr = function
  ...
  | List (_, [Id (l1, "do-while"); e1; e2]) ->
    Begin (l1, (parse_expr e1) :: (While (l1, parse_expr e2, parse_expr e1)) :: [] )
  | List (l, Id (_, "do-while") :: _) -> raise (Syntax_err (l, "invalid \"do-while\""))

  | List (_, Id (l1, "while*") :: texp :: es) ->
    While (l1, parse_expr texp, Begin (l1, (List.map parse_expr es)))
  | List (l, Id (_, "while*") :: _) -> raise (Syntax_err (l, "invalid \"while*\""))

  | List (_, [Id (l1, "for"); bexp; texp; increxpr; List (l2, bexs)]) ->
    Begin (l1, (parse_expr bexp) ::  
      While (l1, parse_expr texp, 
        Begin (l2, (List.map parse_expr bexs) @ (parse_expr increxpr) :: [])) :: [])
  | List (l, Id (_, "for") :: _) -> raise (Syntax_err (l, "invalid \"for\""))

  | List (l, [Id (_, "and"); e1; e2]) -> And (l, parse_expr e1, parse_expr e2)
  | List (l, Id (_, "and") :: _) -> raise (Syntax_err (l, "invalid \"and\""))
  
  | List (l, [Id (_, "or"); e1; e2]) -> Or (l, parse_expr e1, parse_expr e2)
...
let parse_def = function
  ...
  | List (l, [Id (_, "define"); Id (lf, fname); List (_, args); List (_, Id (_, "locals") :: locals); body]) ->
    if List.mem fname keywords
    then raise (Syntax_err (lf, "keywords can't be function names"))
    else
      let args' = List.map to_argname args and
      locals' = List.map to_argname locals in
        (* Check that the formal parameters are unique. *)
        if StringSet.cardinal (StringSet.of_list args') = List.length args'
        then
          if StringSet.cardinal (StringSet.of_list locals') = List.length locals' then
...
let rec eval_expr env exp =
  let module SM = StringMap in   (* local alias *)
    match exp with
      ...
      | While (_, cond, body) ->
        let rec whilei envt = 
          let (env', v) = eval_expr envt cond in
            if v = 0 then  (* we're done *)
              (env', 0)
            else  (* one more time *)
              let (env'', _) = eval_expr env' body in whilei env''
        in whilei env
       ...
       * containing the rest. *)
      | Begin (_, es) ->
        let rec begini envt t = 
          match t with 
          | [] -> (envt, 0)
          | [e] -> eval_expr envt e
          | e :: es' -> let (env', _) = eval_expr envt e in begini env' es'
        in begini env es 

      | And (l, e1, e2) -> 
        let (env', v) = eval_expr env e1 in
        let eval_e1 = Literal (l, v) in        
          eval_expr env' (If (l, eval_e1, e2, eval_e1))

      | Or (l, e1, e2) -> 
        let (env', v) = eval_expr env e1 in
        let eval_e1 = Literal (l, v) in 
        eval_expr env' (If (l, eval_e1, eval_e1, e2))
...
and eval_function loc name f actuals env =
  match f with
    ...
    | UserFunction (formals, locals, body) ->
      let arg_map =
        try
            let zfill = assoc_list_to_map (List.combine formals actuals) in 
              let rec iter vars mp = 
                match vars with
                | [] -> mp
                | h :: [] -> (StringMap.add h 0 mp)
                | h :: t -> iter t (StringMap.add h 0 mp)
              in iter locals zfill
