    ...
    | Call (l, func, args) ->
      ...
      match eval_expr env func with
        ...
        | UserFuncVal (formals, Some rest, body, env') ->
          (* binds actuals to formals as it goes, and binds rest *)
            let env'' = 
              let rec iter twople env = (* actuals. formals) *) 
                match twople with 
                | ([], []) -> Env.bind env rest NilVal
                | ([], _) -> Error.call_err_rest l ~expected:(List.length formals)
                              ~found:(List.length actuals)
                | (lst, []) -> 
                  let ulst = List.fold_right 
                    (fun v lst -> PairVal (ref v, ref lst)) lst NilVal
                  in Env.bind env rest ulst
                | (actual :: ta, formal :: tf) -> 
                    iter (ta, tf) (Env.bind env formal actual)
              in iter (actuals, formals) env'
...
let rec eval_def env def =
  match def with
    ...
    | ValRec (l, bindings) ->
      if bindings = [] then 
        Error.syntax_err l (Printf.sprintf "empty valrec") else
      (* First, assign some placeholders. *)
      let placeholders = 
        List.map (fun (name, _) -> (name, Unspecified name)) bindings 
      in
