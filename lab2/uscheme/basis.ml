...
let prim_cons = binary_fun (fun _ a b -> PairVal (ref a, ref b))
...
let prim_set_car = binary_fun 
  (fun loc v car -> 
     match v with
       | PairVal (a, _) -> begin a := car; UnitVal end 
       | _ -> Error.type_err loc ~expected:"pair" ~found:(string_of_value v))

let prim_set_cdr = binary_fun 
  (fun loc v cdr -> 
     match v with
       | PairVal (_, b) -> begin b := cdr; UnitVal end
       | _ -> Error.type_err loc ~expected:"pair" ~found:(string_of_value v))
(* Printing functions. *)
